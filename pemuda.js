...
const isAntiVO = isGroup ? antiviewonce.includes(from) : false	
		...
		const katalog = (teks) => {
             res = pemuda.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Reza Bot_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pemuda.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pemuda.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© Reza Bot_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Reza Bot_*", [{buttonId: 'simi', buttonText: {disText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
- Hari : ${week} ${weton}
- Tanggal : ${calender}`
sendButLocation(from, `${menu}`, `${uwu}© Reza Bot${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT'},type:1}, {buttonId:`${prefix}ofc`,buttonText:{displayText:'GRUP OWNER'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'allmenu':
        ptod = "6281337541779@s.whatsapp.net"
        stod = `${sender}`
...
│❏${prefix}detikvn <reply audio>
│❏${prefix}detikvideo <reply>
...
└─────────────────⬣
${uwu}`
pemuda.sendMessage(from, menu, text, { quoted: ftrol, thumbnail: fs.readFileSync('./pemuda_1.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtube.com/shorts/Av48uu-Q4hQ?feature=share`}}})
break
case 'menu':
stod = `${sender}`
stst = await pemuda.getStatus(`${sender.split('@')[0]}@c.us`)
...
break
		...
		case 'sc':
		case 'sourcecode':
		pemuda.sendMessage(from, { text: "https://github.com/Frpreza/spebot Nah Tu", matchedText: 'https://youtube.com/shorts/Av48uu-Q4hQ?feature=share', description: "", title: "Yang klik Anak Haram", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
case 'x':
            ...
            "externalAdReply": {
                ...
                "sourceUrl": "https://youtube.com/shorts/Av48uu-Q4hQ?feature=share"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        pemuda.toggleDisappearingMessages(from, 0)
...
pemuda.sendMessage(from, {quoted: ftrol, thumbnail: fs.readFileSync('./media/virgam.jpeg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtube.com/shorts/Av48uu-Q4hQ?feature=share`}}})
break
//•======>> Jan Spam🐦 <<====•\\
case 'bugtroli2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
Giliran = @${tty.er1.split('@')[0]}`
 ...
 //•======>> Jan Spam🐦 <<====•\\
     ...
     pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Reza Bot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./pemuda_1.jpg'),sourceUrl:"https://youtube.com/shorts/Av48uu-Q4hQ?feature=share"}}})
	}
    }
    }
    }
